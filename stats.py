...
def matchfactor(data, criteria, Position, typeoffactor):
    ...
    final_results4.loc[
        (final_results4["Runs"] >= 50) & (final_results4["Runs"] <= 99), "Fifties"
    ] = 1
    final_results4.loc[(final_results4["Runs"] >= 100), "Centuries"] = 1
    final_results4.loc[(final_results4["BF"] > 0), "RunswithBalls"] = final_results4[
        "Runs"
    ]
    ...
    if choice4:
        final_results5 = final_results5[
            final_results5["Batting_Position"].isin(choice4)
        ]
    ...
    final_results5["Season"] = final_results5["SeriesName"].str.extract(
        r", (\d{4}(?:/\d{2})?)$"
    )
    ...
    final_results5 = final_results5[
        ~final_results5["SeriesName"].isin(
            [
                ...
                "Triangular Tournament (Australia, England, South Africa in England), 1912",
            ]
        )
    ]

    df_match_totals = (
        final_results5.groupby(
            [
                "New Batter",
                "Team",
                "PlayerID",
    ...
    if typeoffactor == "Team and Opposition":
        ...
        df_match_totals2 = (
            final_results4.groupby(["Start_Date", "Host Country", "year"])
            .agg(
                Runs=("Runs", "sum"),
                Outs=("Out", "sum"),
                Balls=("BF", "sum"),
        ...
        batting = pd.merge(
            df_match_totals,
            df_match_totals2,
            on=["Team", "Start_Date", "Host Country", "year"],
            suffixes=("", "_grouped"),
        )

    batting["cen_diff"] = batting["Centuries_grouped"] - batting["Centuries"]
    batting["FiftiesPlus_diff"] = (
        batting["Fifties_grouped"]
        + batting["Centuries_grouped"]
        - batting["Fifties"]
        - batting["Centuries"]
    )
    batting["run_diff"] = batting["Runs_grouped"] - batting["Runs"]
    batting["runswithballs_diff"] = (
        batting["RunswithBalls_grouped"] - batting["RunswithBalls"]
    )
    ...
    batting.loc[batting["FiftiesPlus_diff"] == start_runs, "FiftyPlusScored"] = (
        "0 FiftyPluses"
    )
    ...
    final_results5["sr"] = (
        (final_results5["RunswithBalls"]) / (final_results5["Balls"]) * 100
    )
    final_results5["mean_ave"] = (
        (final_results5["run_diff"]) / (final_results5["out_diff"])
    )
    final_results5["mean_sr"] = (
        (final_results5["runswithballs_diff"]) / (final_results5["ball_diff"]) * 100
    )
    final_results5["Match Factor"] = (
        (final_results5["ave"]) / (final_results5["mean_ave"])
    )
    final_results5["Strike Factor"] = (
        (final_results5["sr"]) / (final_results5["mean_sr"])
    )
    ...
    final_results5 = final_results5[
        (final_results5["Runs"] >= start_runs) & (final_results5["Runs"] <= end_runs)
    ]
    balls = max((final_results5["Balls"]).astype(int))
    start_balls, end_balls = st.sidebar.slider(
        "Select Minimum Balls:", min_value=0, max_value=balls, value=(0, balls)
    )
...
def bowlmatchfactor(bowling, criteria):
    ...
    typeoffactor = st.sidebar.selectbox(
        "Select Match Factor by Team or Team and Opposition:",
        ["Team and Opposition", "Team"],
    )
    # bowling=bowling[bowling['Bowling Position']<=4]
    typeoftype = st.sidebar.selectbox(
        "Select Match Factor by BowlType or Overall:", ["BowlType", "Overall"]
    )
    bowling2 = bowling

    choice4 = st.sidebar.multiselect(
        "Bowling Position Baseline:", bowling["Bowling_Position"].unique()
    )
    if choice4:
    ...
    bowling2["Season"] = bowling2["Season"].fillna(bowling2["Start_Date"].dt.year)
    bowling2 = bowling2[
        ~bowling2["SeriesName"].isin(
            [
                "ICC World Test Championship, 2019-2021",
                "ICC World Test Championship, 2021-2023",
                "ICC World Test Championship, 2023-2025",
    ...
    df_match_totals.loc[df_match_totals["Wickets"] >= 9, "Ninefer"] = 1
    ...
    if typeoffactor == "Team and Opposition":
        ...
        if typeoftype == "BowlType":
            df_match_totals2 = (
                bowling.groupby(
                    ["Start_Date", "BowlType", "Ground", "Host Country", "year"]
                )
                .agg(
                    Runs=("Runs", "sum"),
                    Balls=("Balls", "sum"),
            ...
            bowling = pd.merge(
                df_match_totals,
                df_match_totals2,
                on=["Start_Date", "BowlType", "Ground", "Host Country", "year"],
                suffixes=("", "_grouped"),
            )
        else:
            df_match_totals2 = (
                bowling.groupby(["Start_Date", "Ground", "Host Country", "year"])
                .agg(
                    Runs=("Runs", "sum"),
                    Balls=("Balls", "sum"),
                    Wickets=("Wkts", "sum"),
            ...
            bowling = pd.merge(
                df_match_totals,
                df_match_totals2,
                on=["Start_Date", "Ground", "Host Country", "year"],
                suffixes=("", "_grouped"),
            )
    else:
        if typeoftype == "BowlType":
            df_match_totals2 = (
                bowling.groupby(
                    ["Team", "Start_Date", "BowlType", "Ground", "Host Country", "year"]
                )
                .agg(
                    Runs=("Runs", "sum"),
                    Balls=("Balls", "sum"),
            ...
            bowling = pd.merge(
                df_match_totals,
                df_match_totals2,
                on=["Team", "Start_Date", "BowlType", "Ground", "Host Country", "year"],
                suffixes=("", "_grouped"),
            )
        else:
            df_match_totals2 = (
                bowling.groupby(
                    ["Team", "Start_Date", "Ground", "Host Country", "year"]
                )
                .agg(
                    Runs=("Runs", "sum"),
                    Balls=("Balls", "sum"),
            ...
            bowling = pd.merge(
                df_match_totals,
                df_match_totals2,
                on=["Team", "Start_Date", "Ground", "Host Country", "year"],
                suffixes=("", "_grouped"),
            )
...
def main():
    ...
    if choice0 == "Batting":
        ...
        filtered_data2 = filtered_data[
            (filtered_data["Start_Date"] >= pd.to_datetime(start_date))
            & (filtered_data["Start_Date"] <= pd.to_datetime(end_date))
        ]
        filtered_data2["year"] = pd.to_datetime(
            filtered_data2["Start_Date"], format="mixed"
        ).dt.year
        ...
        peaksornot = st.sidebar.selectbox("Bowling Peaks:", ["No", "Yes"])
        if peaksornot == "No":
            start_date = st.sidebar.date_input(
                "Start date", min_value=min_dt, max_value=max_dt, value=min_dt
            )
            ...
            choice4 = st.sidebar.multiselect(
                "Host Country:", data["Host Country"].unique()
            )
            # choice5 = st.sidebar.multiselect('Team:', data['Team'].unique())
            #    Filtering data based on the user's Date selection
            start_runs, end_runs = st.sidebar.slider(
                "Select Minimum Wickets:", min_value=1, max_value=run, value=(1, run)
            )
            filtered_data = data
            filtered_data2 = filtered_data[
                (filtered_data["Start_Date"] >= pd.to_datetime(start_date))
            ...
            if choice3:
                filtered_data2 = filtered_data2[
                    filtered_data2["BowlType"].isin(choice3)
                ]
            if choice4:
                filtered_data2 = filtered_data2[
                    filtered_data2["Host Country"].isin(choice4)
                ]
            # if choice5:
            #     filtered_data2 = filtered_data2[filtered_data2['Team'].isin(choice5)]
            choice5 = st.sidebar.selectbox(
            ...
            choice = st.sidebar.selectbox(
                "Do you want Peaks by Number of Innings or Wickets:", options
            )
            if choice:
                choice2 = st.sidebar.selectbox(
                    "Individual Player or Everyone:", ["Individual", "Everyone"]
                ...
                if choice == "N Number of Wickets":
                    start_runs = st.sidebar.slider(
                        "Select the wicket threshold for peaks:",
                        min_value=1,
                        max_value=wkts,
                    )
                else:
                    start_runs = st.sidebar.slider(
                        "Select the innings threshold for peaks:",
                        min_value=1,
                ...
                final_results4 = (
                    filtered_data2.groupby(["BowlType", "Start_Date", "Host Country"])
                    .agg(
                        InnsTotal=("I", "sum"),
                        RunsTotal=("Runs", "sum"),
                        OutsTotal=("Wkts", "sum"),
                ...
                data2 = pd.merge(
                    filtered_data2,
                    final_results4,
                    on=["BowlType", "Start_Date", "Host Country"],
                    how="left",
                )
                ...
                for batter, group in data2.groupby("Bowler"):
                    if choice == "N Number of Innings":
                        ...
                        peak_matches = find_lowest_average_200_wickets_with_leeway(
                            group, start_runs
                        )
                ...
                streaks_summary["Ave"] = (
                    streaks_summary["Runs"] / streaks_summary["Wkts"]
                )
                streaks_summary["Mean Ave"] = (
                    streaks_summary["run_diff"] / streaks_summary["out_diff"]
                )

                streaks_summary["SR"] = (
                    streaks_summary["Balls"] / streaks_summary["Wkts"]
                )
                streaks_summary["Mean SR"] = (
                    streaks_summary["ball_diff"] / streaks_summary["out_diff"]
                )

                streaks_summary["Match Factor"] = (
                    streaks_summary["Mean Ave"] / streaks_summary["Ave"]
                )
                streaks_summary["Strike Rate Factor"] = (
                    streaks_summary["Mean SR"] / streaks_summary["SR"]
                )

                streaks_summary = streaks_summary[streaks_summary["ispeak"] == "Peak"]
                streaks_summary = streaks_summary.drop(
                    columns=[
                        "PlayerID",
                        "ispeak",
                        "inn_diff",
                ...
                if choice2 == "Individual":
                    ...
                    streaks_summary = streaks_summary[
                        streaks_summary["Bowler"].isin(temp)
                    ]
                    streaks_summary = streaks_summary.rename(
                        columns={"Bowler": "Bowler"}
                    )
                    ...
                    streaks_summary = streaks_summary.sort_values(
                        by=["Match Factor"], ascending=False
                    )
                    st.dataframe(streaks_summary.round(2), use_container_width=True)
