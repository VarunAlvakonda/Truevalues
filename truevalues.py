...
#             # pl.when(pl.col('BF') == 0)
#             # .then(pl.lit(0.0))
#             # .otherwise((pl.col('Out') - pl.col('Expected Outs')) / (pl.col('BF') / 360))
#             # .alias('True W/ 30 overs')
#         ]
#     )
...
#     first_appearance = data.unique(
#         subset=["MatchNum", "TeamInns", "Batter"], keep="first"
#     )
...
#     avg_entry_point_deliveries = first_appearance.group_by(["Batsman", "Batter"]).agg(
#         pl.col("total_deliveries").median()
#     )

#     # Convert back to overs
#     avg_entry_point_deliveries = avg_entry_point_deliveries.with_columns(
#         [
#             ((pl.col("total_deliveries") // 6) + (pl.col("total_deliveries") % 6) / 10)
#             .round(1)
#             .alias("average_over")
#         ]
#     )

#     return avg_entry_point_deliveries.select(
#         ["Batsman", "Batter", "average_over"]
#     ), first_appearance
...
#     avg_entry_point_deliveries = first_appearance.group_by(
#         ["Batsman", "Batter", "year"]
...
#     # Early filtering
#     combineddata = data2.filter((pl.col("TeamInns") < 3) & (pl.col("year") == year2))
...
#     # Cumulative innings
#     inns = inns.with_columns(pl.col("I").cum_sum().over("Batter").alias("CI"))
...
#     player_runs = (
#         combineddata.group_by(groupby_cols_player)
#         .agg(
#             [
...
#     over_outs = (
#         dismissed_data.group_by(groupby_cols_over)
...
#     over_runs = (
#         combineddata.group_by(groupby_cols_over)
...
#         [pl.col(col).sum() for col in agg_cols]
#     )
...
#     # Cumulative innings
#     inns = inns.with_columns(pl.col("I").cum_sum().over("Bowlers").alias("CI"))
...
#     # Over-level aggregations
#     over_outs = (
#         dismissed_data.group_by([place, "over"])
#         .agg(pl.col("Out").sum().alias("Outs"))
#         .select([pl.col(place), pl.col("over").alias("Over"), pl.col("Outs")])
#     )

#     over_runs = (
#         combineddata.group_by([place, "over"])
...
#     #     "Select Format:", ["T20s", "ODI", "WT20s", "WODI"]
#     # )
#     format_choice = st.sidebar.selectbox(
#         "Select Format:",
#         [
...
#         "Choose Competitions:", data["CompName"].unique(maintain_order=True).to_list()
#     )
...
#     elif (
#         format_choice == "T20s"
#         and selected_leagues
#         and "Twenty20 International" not in selected_leagues
#     ):
#         place = "Venue_x"
#     else:
#         place = "Country"
...
#         (pl.col("Date") >= start_datetime) & (pl.col("Date") <= end_datetime)
#     )
...
#                     "Impact",
#                 ]
#                 truevalues = combined_data.group_by(["Player", "Batsman"]).agg(
#                     [pl.col(col).sum() for col in agg_cols]
#                 )
#                 final_results = truemetrics(truevalues)
...
#                     "Impact",
#                 ]
#                 truevalues = combined_data.group_by(["Player", "BowlNum", "Type"]).agg(
#                     [pl.col(col).sum() for col in agg_cols]
#                 )
#                 final_results = truemetricsbowling(truevalues)
...
#                     valid_players = set(
#                         final_results["Player"].unique(maintain_order=True).to_list()
#                     )
...
#                     start_runs, end_runs = st.slider(
#                         "Select Wickets:", min_value=0, max_value=outs, value=(1, outs)
#                     )
#                     start_balls, end_balls = st.slider(
#                         "Select Minimum BF:",
#                         min_value=1,
...
#                 if choice2 == "Individual":
#                     valid_players = set(
#                         combined_data["Player"].unique(maintain_order=True).to_list()
#                     )
#                     temp = [p for p in player if p in valid_players]
...
def analyze_data_for_year3(year2, player_data, baseline_data):
    """Analyze batting data for a specific year - Polars version"""
    print(f"Processing year {year2} (batting analyze_3)...")

    # Player-level data — fully filtered (pace/spin, innings, overs etc.)
    combineddata = player_data.filter(
        (pl.col("TeamInns") < 3) & (pl.col("year") == year2)
    )

    # Baseline data — only date/competition/overs filtered; used for over-level rates
    baseline = baseline_data.filter(
        (pl.col("TeamInns") < 3) & (pl.col("year") == year2)
    )
    ...
    # Dismissal data — from player_data
    valid = ["X", "WX"]
    dismissed_data = combineddata.filter(pl.col("Notes").is_in(valid))
    dismissed_data = dismissed_data.with_columns(pl.lit(1).alias("Out"))
    ...
    over_outs = (
        baseline_dismissed.group_by(groupby_cols_over)
        .agg(pl.col("Out").sum().alias("Outs"))
        .select(
            [
                pl.col("TeamInns"),
    ...
    over_runs = (
        baseline.group_by(groupby_cols_over)
        .agg([pl.col("Runs").sum(), pl.col("B").sum()])
        .select(
            [
                pl.col("TeamInns"),
...
def analyze_data_for_year6(year2, player_data, baseline_data):
    """Analyze bowling data for a specific year - Polars version"""
    print(f"Processing year {year2} (bowling analyze_6)...")
...
def main():
    ...
    # Date-filtered data — root for both baseline and player branches
    filtered_data2 = data.filter(
        (pl.col("Date") >= start_datetime) & (pl.col("Date") <= end_datetime)
    )
    ...
    choice4 = st.sidebar.multiselect("Select Innings:", [1, 2])
    if choice0 == "Batting":
        max_balls = int(filtered_data2["BatterBalls"].cast(pl.Int64).max())
        choice5 = start_balls, end_balls = st.sidebar.slider(
            "Select Balls Faced in Innings:",
            min_value=1,
            max_value=max_balls,
            value=(0, max_balls),
        )
    ...
    baseline_data = filtered_data2.filter(
        (pl.col("over") >= start_over) & (pl.col("over") <= end_over)
    )

    player_data = baseline_data
    if choice3:
        player_data = player_data.filter(pl.col("BowlCat").is_in(choice3))
    if choice4:
        player_data = player_data.filter(pl.col("TeamInns").is_in(choice4))
    if choice0 == "Batting":
        if choice5:
            player_data = player_data.filter(
                (pl.col("BatterBalls") >= start_balls)
                & (pl.col("BatterBalls") <= end_balls)
            )
        if choice6:
    ...
    with st.sidebar.form("results_form"):
        ...
        if show_results:
            ...
            for i, year in enumerate(unique_years):
                if choice0 == "Batting":
                    results = analyze_data_for_year3(year, player_data, baseline_data)
                else:
                    results = analyze_data_for_year6(year, player_data, baseline_data)
                all_data.append(results)
